...
export default function DashboardPage() {
  ...
  // Cálculos de cobros y totales
  const totalCobrado = pedidos.reduce((sum, p) => {
    if (p.pago && p.pago.estado === 'pagado') {
      return sum + Number(p.pago.monto)
    }
    return sum
  }, 0)
  ...
  return (
    <main className="page-content p-6 max-w-7xl mx-auto text-white space-y-8">
                  ...
                  {pedidos.map((pedido) => (
                    <tr key={pedido.id} className="hover:bg-slate-800/40 text-slate-300 transition duration-150">
                      ...
                      <td className="px-6 py-4">
                        {pedido.pago ? (
                          <div className="flex flex-col">
                            <span className="font-semibold text-white font-mono">{formatCurrency(pedido.pago.monto)}</span>
                            <span className={`inline-flex items-center self-start px-1.5 py-0.5 rounded text-[10px] font-bold uppercase mt-1 ${pedido.pago.estado === 'pagado'
                                ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
                                : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'
                              }`}>
                              {pedido.pago.estado === 'pagado' ? 'Cobrado' : 'Por Cobrar'}
                            </span>
                          </div>
                        ) : (
                          <span className="text-slate-600 italic text-xs">Sin Pago</span>
                        )}
                      </td>
      ...
      {/* <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-xl overflow-hidden">
        <div className="panel-header border-b border-slate-800 px-6 py-4">
          <span className="panel-title text-base font-bold text-slate-300">Usuarios Registrados en la Base de Datos</span>
        </div>
        <div className="panel-body p-0">
          {loadingUsers ? (
            <div className="py-20 flex flex-col items-center justify-center text-slate-400 gap-3">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-800 text-sm">
                  {users.map((u) => (
                    <tr key={u.id} id={`user-row-${u.id}`} className="hover:bg-slate-800/40 text-slate-300 transition duration-150">
                      <td className="px-6 py-4 font-mono font-bold text-slate-500 text-xs">#{u.id}</td>
                      <td className="px-6 py-4 flex items-center gap-3">
                        <div className="flex items-center justify-center w-8 h-8 rounded-full bg-slate-800 border border-slate-700 text-white font-black text-xs">
                      ...
                      <td className="px-6 py-4 text-xs text-slate-500">{u.created_at}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
